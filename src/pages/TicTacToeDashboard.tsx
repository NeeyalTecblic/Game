...
const TicTacToeDashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const playerWins = history.filter((game: GameResult) => game.winner === 'Player').length;
    const computerWins = history.filter((game: GameResult) => game.winner === 'Computer').length;
    const draws = history.filter((game: GameResult) => game.winner === 'Draw').length;
    const totalMoves = history.reduce((sum: number, game: GameResult) => sum + game.moves, 0);
